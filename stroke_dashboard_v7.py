...
if menu == "🤖 Prediksi Stroke":
    ...
    if predict_btn:
        ...
        # Radar chart — patient profile vs average
        st.subheader("🕸️ Profil Pasien vs Rata-rata Dataset")
        avg = df.mean(numeric_only=True)
        categories = ['Usia (norm)', 'Glukosa (norm)', 'BMI (norm)', 'Hipertensi', 'P.Jantung']
        patient_vals = [
            age/100, glucose/300, bmi/50,
            float(hypertension), float(heart_disease)
        ]
        avg_vals = [
            avg['age']/100, avg['avg_glucose_level']/300, avg['bmi']/50,
            avg['hypertension'], avg['heart_disease']
        ]
        ...
        fig_radar.add_trace(go.Scatterpolar(
            ...
            theta=categories + [categories[0]],
            fill='toself', name='Rata-rata Dataset',
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    stat_var = st.selectbox("Pilih variabel untuk dilihat distribusinya:",
                             ...
                             format_func=lambda x: {"age":"Usia (tahun)","avg_glucose_level":"Glukosa (mg/dL)","bmi":"BMI"}[x])
    ...
    with c1:
        fig_hist = px.histogram(df, x=stat_var, color='stroke_label', barmode='overlay',
                                 ...
                                 labels={stat_var: stat_var, 'stroke_label':'Status'},
                                 title=f"Distribusi {stat_var} per Status Stroke")
        fig_hist.update_layout(height=300, **PLOTLY_LAYOUT, bargap=0.05)
        st.plotly_chart(fig_hist, use_container_width=True)

    with c2:
        fig_box = px.box(df, x='stroke_label', y=stat_var, color='stroke_label',
                          color_discrete_map=STROKE_COLOR_MAP, points="outliers",
                          title=f"Box Plot {stat_var}",
                          labels={'stroke_label':'Status', stat_var: stat_var})
        fig_box.update_layout(height=300, **PLOTLY_LAYOUT, showlegend=False)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        st.markdown("---")
        st.subheader("📊 Violin Plot — Perbandingan Distribusi")
        viol_var = st.selectbox("Variabel:", num_cols,
                                 format_func=lambda x: num_labels[x], key="viol_var")
        fig_viol = px.violin(df, x='stroke_label', y=viol_var, color='stroke_label',
                              color_discrete_map=STROKE_COLOR_MAP, box=True, points="outliers",
                              labels={'stroke_label':'Status', viol_var: num_labels[viol_var]},
                              title=f"Violin Plot — {num_labels[viol_var]}")
        fig_viol.update_layout(height=380, **PLOTLY_LAYOUT, showlegend=False)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_ab:
        ...
        fig_cont.update_layout(title="Heatmap Tabel Kontingensi", height=300, **PLOTLY_LAYOUT)
